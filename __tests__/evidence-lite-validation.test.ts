...
describe("evidence lite contract validation", () => {
  ...
  it("rejects immutable fields and empty update payloads", () => {
    const immutableResult = validateUpdateEvidenceLiteInput({
      id: "evi-1",
      dealId: "deal-123",
    ...
    expect(immutableResult.errors.map((error) => error.field)).toEqual(
      expect.arrayContaining(["id", "dealId", "createdAt", "updatedAt", "root"])
    )
  ...
  it("accepts a focused update payload and trims mutable text fields", () => {
